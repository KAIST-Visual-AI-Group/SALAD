...
from pvd.modules.transformer import TimeMLP
import math


class ConcatDecoder(nn.Module):
    def __init__(self, dim_self, dim_ref, dim_ctx, num_layers=3):
        super().__init__()
        dim_total = dim_self + dim_ref

        self.layers = nn.ModuleList(
            [
                TimeMLP(dim_total, dim_total, dim_total, dim_ctx, use_time=True)
                for _ in range(num_layers)
            ]
        )
        self.fc = nn.Linear(dim_total, dim_self)
