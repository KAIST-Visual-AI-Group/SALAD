...
# def add_spaghetti_path(spaghetti_path="/home/juil/projects/3D_CRISPR/spaghetti_github"):
def add_spaghetti_path(spaghetti_path="/home/juil/docker_home/projects/3D_CRISPR/spaghetti_github"):
    if spaghetti_path not in sys.path:
        sys.path.append(spaghetti_path)

# def delete_spaghetti_path(spaghetti_path="/home/juil/projects/3D_CRISPR/spaghetti_github"):
def delete_spaghetti_path(spaghetti_path="/home/juil/docker_home/projects/3D_CRISPR/spaghetti_github"):
    if spaghetti_path in sys.path:
        sys.path.remove(spaghetti_path)

def load_spaghetti(device, tag="chairs_large"):
    add_spaghetti_path()
    from pvd.spaghetti_github.options import Options
    from pvd.spaghetti_github.ui import occ_inference
...
def load_mesher(device, min_res=64,):
    from pvd.spaghetti_github.utils.mcubes_meshing import MarchingCubesMeshing
...
def generate_zc_from_sj_gaus(spaghetti, sj: Union[torch.Tensor, np.ndarray], gaus: Union[torch.Tensor, np.ndarray]):
    """
    Input:
...
def generate_zc_from_single_phase_latent(spaghetti, sj_gaus: Union[torch.Tensor, np.ndarray]):
    device = spaghetti.device
...
def reflect_and_concat_gmms(gmms: torch.Tensor):
    ...
    assert gmms.shape == gmms_t.shape, "Input and reflected gmms shapes must be the same"
...
def project_eigenvectors(gaus: Union[torch.Tensor, np.ndarray]):
    ...
    """
    gaus = jutils.nputil.np2th(gaus).clone()
    if gaus.ndim == 2:
...
def save_meshes_and_pointclouds(spaghetti, mesher, zcs, save_top_dir, mesh_save_dir=None, pc_save_dir=None, num_shapes=2000):
